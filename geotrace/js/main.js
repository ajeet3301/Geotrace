...
(function () {
  ...
  if (dot && ring) {
    ...
    document.querySelectorAll('a, button, .feature-card, .testimonial-card, .marquee-item, .upload-zone').forEach((el) => {
  ...
  function animateCounters() {
    document.querySelectorAll('[data-count]').forEach((el) => {
      ...
      (function step(now) {
        ...
        if (target >= 1000000) el.textContent = (cur/1000000).toFixed(1) + 'M' + suffix;
        else if (target >= 1000) el.textContent = Math.round(cur).toLocaleString() + suffix;
        else if (target % 1 !== 0) el.textContent = cur.toFixed(1) + suffix;
  ...
  if (heroStats && 'IntersectionObserver' in window) {
    let fired = false;
    const io = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !fired) { fired = true; animateCounters(); io.disconnect(); }
    }, { threshold: 0.3 });
  ...
  if (tw) {
    ...
    function type() {
      const word = words[wi];
      if (deleting) {
        ci--; tw.textContent = word.substring(0,ci);
  ...
  window.GeoTrace.toast = function (msg, type='info', duration=4000) {
    ...
    toast.style.cssText = `padding:12px 20px;border-radius:10px;background:rgba(8,13,24,0.95);border:1px solid ${colors[type]||colors.info};color:var(--text-primary);font-size:14px;font-family:var(--font-body);box-shadow:0 4px 24px rgba(0,0,0,0.4);animation:toast-in 0.3s ease;max-width:340px;display:flex;align-items:center;gap:10px;`;
