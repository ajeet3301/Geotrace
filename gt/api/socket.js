...
export default function handler(req, res) {
  ...
  if (req.method==='GET'  && path==='/analytics') {
    ...
    for (let i=1;i<pts.length;i++){dist+=haversine(pts[i-1],pts[i]);maxSpeed=Math.max(maxSpeed,pts[i].speed||0)}
    const dur=pts[pts.length-1].ts-pts[0].ts;
    return res.json({distance:+dist.toFixed(2),duration:Math.round(dur/60000),
      avgSpeed:+(dist/(dur/3600000)).toFixed(1),maxSpeed:+maxSpeed.toFixed(1),
      points:pts.length,startTime:pts[0].ts,endTime:pts[pts.length-1].ts});
  }
...
function updateHeatmap(uid,lat,lon){
  ...
  const C=0.0005,key=`${Math.round(lat/C)*C},${Math.round(lon/C)*C}`;
...
function checkFences(uid,lat,lon){
  return Object.values(store.geofences).filter(f=>f.ownerId===uid).reduce((acc,f)=>{
    const dist=haversine({lat,lon},{lat:f.lat,lon:f.lon})*1000,key=`${uid}:${f.id}`;
    const was=fenceState[key]||false,is=dist<=f.radius;
